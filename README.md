...
在这一部分，你需要实现通行版规则中电脑端 C 所要扮演的角色。为了方便我们在 `ACMOJ`上提交，我们对规则生成机制进行了一些修改，比如引入了随机数种子，引入了撤销机制（最多一次，不能连续），不再限制2048的棋盘为 4 \* 4等等。总体来说，你需要实现`game_logic.cpp`中的所有函数，具体功能详见`game_logic.h`与`game_logic.cpp`.
...
对于 Extension 部分，由于`ACMOJ`的性能较差，这一部分最终得分取决于你代码在本地测试的 Average Score，对此，我们制定了以下公式用来换算你在本部分获得的分数:
...
感谢2025级赵睿城指出项目中 `Undo()` 函数的错误，特此感谢。
