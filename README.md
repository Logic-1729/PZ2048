...
在这一部分，你需要实现通行版规则中电脑端 C 所要扮演的角色。为了方便我们在 `ACMOJ`上提交，我们对规则生成机制进行了一些修改，比如引入了随机数种子，引入了撤销机制（上限一次），不再限制2048的棋盘为 4 \* 4等等。总体来说，你需要实现`game_logic.cpp`中的所有函数，具体功能详见`game_logic.h`与`game_logic.cpp`.
